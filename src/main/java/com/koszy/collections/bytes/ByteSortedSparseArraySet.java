...
public class ByteSortedSparseArraySet extends AbstractByteSortedSet implements Cloneable {
	...
	@Override
	public ByteSortedSparseArraySet clone() {
		final ByteSortedSparseArraySet c;
		try {
			c = (ByteSortedSparseArraySet) super.clone();
		...
		SetIterator(final byte from) {
			...
			switch (ByteSortedSparseArraySet.this.size) {
				...
				case 1 -> {
					...
					final int compare = ByteSortedSparseArraySet.this.compare(from, key[last]);
