package com.koszy.collections.floats;
...
public class FloatSortedSparseArraySet extends AbstractFloatSortedSet implements Cloneable {
	...
	public FloatSortedSparseArraySet() {
		this(
	...
	public FloatSortedSparseArraySet(final int expected) {
		this(
				expected,
				Hash.DEFAULT_LOAD_FACTOR,
				null,
	...
	public FloatSortedSparseArraySet(final FloatComparator comparator) {
		this(
				Hash.DEFAULT_INITIAL_SIZE,
				Hash.DEFAULT_LOAD_FACTOR,
				comparator,
	...
	public FloatSortedSparseArraySet(final RandomGenerator random) {
		this(
				Hash.DEFAULT_INITIAL_SIZE,
				Hash.DEFAULT_LOAD_FACTOR,
				null,
	...
	protected int compare(final float k1, final float k2) {
	...
	public void clear() {
		...
		this.size = 0;
		Arrays.fill(this.key, Float.intBitsToFloat(0));
	...
	private int insert(final float k, final int low, final int high) {
		final float[] key = this.key;
		...
		if (high == begin) {
			...
			slot = pos - 1;
			nulll = Float.floatToRawIntBits(last) == 0 ? slot : this.nulll;

			while (Float.floatToRawIntBits(last) != 0 || slot == nulll) {
				curr = key[++slot];
				key[slot] = last;
				if (Float.floatToRawIntBits(last) == 0) {
					this.nulll = slot;
				}
				last = curr;
			}
			...
			if (this.random.nextBoolean()) { // Shift right
				...
				while (slot < end && (Float.floatToRawIntBits(last) != 0 || slot == nulll)) {
					curr = key[++slot];
					key[slot] = last;
					if (Float.floatToRawIntBits(last) == 0) {
						this.nulll = slot;
				...
				if (Float.floatToRawIntBits(last) == 0 && slot != nulll) {
					this.last = Math.max(slot, this.last);
				...
				while (begin < slot && (Float.floatToRawIntBits(last) != 0 || slot == nulll)) {
					curr = key[--slot];
					key[slot] = last;
					if (Float.floatToRawIntBits(last) == 0) {
						this.nulll = slot;
	...
	protected void fixPointers(final int i) {
		switch (this.size) {
			...
			default -> {
				...
				if (i == this.first) {
					while (Float.floatToRawIntBits(key[++this.first]) == 0 && this.first != nulll) {
					}
				} else if (i == this.last) {
					while (Float.floatToRawIntBits(key[--this.last]) == 0 && this.last != nulll) {
					}
				}
			}
		}
	...
	protected void removeEntry(final int pos) {
		...
		this.key[pos] = Float.intBitsToFloat(0);
	...
	public void forEach(final FloatConsumer action) {
		...
		while (next != -1) {
			curr = next;
			while (++next < last && (Float.floatToRawIntBits(key[next]) == 0 && next != nulll)) {
			}
			if (last < next) {
				next = -1;
			}
	...
	protected void resort(final int newN) {
		...
		while (0 < required) {
			if (random.nextInt(--j) < required) {
				float o;
				while (Float.floatToRawIntBits(o = key[--i]) == 0 && i != nulll) {
				}
	...
	protected long sparseBinarySearch(final float k) {
		final float[] key = this.key;
		...
		while (true) {
			...
			int sign = -1;
			int distance = 0;
			while (Float.floatToRawIntBits(o = key[mid]) == 0 && mid != nulll) {
				sign = -sign;
				mid += (sign * ++distance);
			}
	...
	private final class SetIterator implements FloatBidirectionalIterator {
		int prev = -1;
		int next = -1;
		int curr = -1;

		SetIterator() {
			this.next = FloatSortedSparseArraySet.this.first;
		}

		SetIterator(final float from) {
			final float[] key = FloatSortedSparseArraySet.this.key;
			switch (FloatSortedSparseArraySet.this.size) {
				...
				default -> {
					final int nulll = FloatSortedSparseArraySet.this.nulll;
					final int first = FloatSortedSparseArraySet.this.first;
					final int last = FloatSortedSparseArraySet.this.last;
					...
					final long packed = FloatSortedSparseArraySet.this.sparseBinarySearch(from);
		...
		public void remove() {
			...
			if (this.next == -1) {
				FloatSortedSparseArraySet.this.last = this.prev;
			}

			key[pos] = Float.intBitsToFloat(0);
		}
