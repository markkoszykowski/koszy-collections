package com.koszy.collections.longs;
...
public class LongSortedSparseArraySet extends AbstractLongSortedSet {
	...
	public LongSortedSparseArraySet() {
		this(
	...
	public LongSortedSparseArraySet(final int expected) {
		this(
				expected,
				Hash.DEFAULT_LOAD_FACTOR,
				null,
	...
	public LongSortedSparseArraySet(final LongComparator comparator) {
		this(
				Hash.DEFAULT_INITIAL_SIZE,
				Hash.DEFAULT_LOAD_FACTOR,
				comparator,
	...
	public LongSortedSparseArraySet(final RandomGenerator random) {
		this(
				Hash.DEFAULT_INITIAL_SIZE,
				Hash.DEFAULT_LOAD_FACTOR,
				null,
	...
	protected int compare(final long k1, final long k2) {
	...
	private int insert(final long k, final int low, final int high) {
		final long[] key = this.key;
		...
		if (high == begin) {
			...
			slot = pos - 1;
			nulll = last == 0L ? slot : this.nulll;

			while (last != 0L || slot == nulll) {
				curr = key[++slot];
				key[slot] = last;
				if (last == 0L) {
					this.nulll = slot;
			...
			if (this.random.nextBoolean()) { // Shift right
				...
				while (slot < end && (last != 0L || slot == nulll)) {
					curr = key[++slot];
					key[slot] = last;
					if (last == 0L) {
						this.nulll = slot;
				...
				slot = pos + 1;
				nulll = k == 0L ? slot : this.nulll;

				while (begin < slot && (last != 0L || slot == nulll)) {
					curr = key[--slot];
					key[slot] = last;
					if (last == 0L) {
						this.nulll = slot;
	...
	protected void fixPointers(final int i) {
		switch (this.size) {
			...
			default -> {
				...
				if (i == this.first) {
					while (key[++this.first] == 0L && this.first != nulll) {
					}
				} else if (i == this.last) {
					while (key[--this.last] == 0L && this.last != nulll) {
					}
				}
			}
		}
	...
	public void forEach(final LongConsumer action) {
		...
		while (next != -1) {
			curr = next;
			while (++next < last && (key[next] == 0L && next != nulll)) {
			}
			if (last < next) {
				next = -1;
			}
	...
	protected long sparseBinarySearch(final long k) {
		final long[] key = this.key;
	...
	private final class SetIterator implements LongBidirectionalIterator {
		...
		SetIterator() {
			this.next = LongSortedSparseArraySet.this.first;
		}

		SetIterator(final long from) {
			final long[] key = LongSortedSparseArraySet.this.key;
			switch (LongSortedSparseArraySet.this.size) {
				...
				default -> {
					final int nulll = LongSortedSparseArraySet.this.nulll;
					final int first = LongSortedSparseArraySet.this.first;
					final int last = LongSortedSparseArraySet.this.last;
					...
					final long packed = LongSortedSparseArraySet.this.sparseBinarySearch(from);
		...
		public void remove() {
			...
			if (this.next == -1) {
				LongSortedSparseArraySet.this.last = this.prev;
			}
