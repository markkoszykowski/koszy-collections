package com.koszy.collections.ints;
...
public class IntSortedSparseArraySet extends AbstractIntSortedSet implements Cloneable {
	...
	public IntSortedSparseArraySet() {
		this(
	...
	public IntSortedSparseArraySet(final int expected) {
		this(
				expected,
				Hash.DEFAULT_LOAD_FACTOR,
				null,
	...
	public IntSortedSparseArraySet(final IntComparator comparator) {
		this(
				Hash.DEFAULT_INITIAL_SIZE,
				Hash.DEFAULT_LOAD_FACTOR,
				comparator,
	...
	public IntSortedSparseArraySet(final RandomGenerator random) {
		this(
				Hash.DEFAULT_INITIAL_SIZE,
				Hash.DEFAULT_LOAD_FACTOR,
				null,
	...
	protected int compare(final int k1, final int k2) {
	...
	private int insert(final int k, final int low, final int high) {
		final int[] key = this.key;
		...
		if (high == begin) {
			...
			slot = pos - 1;
			nulll = last == 0 ? slot : this.nulll;

			while (last != 0 || slot == nulll) {
				curr = key[++slot];
				key[slot] = last;
				if (last == 0) {
					this.nulll = slot;
			...
			if (this.random.nextBoolean()) { // Shift right
				...
				while (slot < end && (last != 0 || slot == nulll)) {
					curr = key[++slot];
					key[slot] = last;
					if (last == 0) {
						this.nulll = slot;
				...
				while (begin < slot && (last != 0 || slot == nulll)) {
					curr = key[--slot];
					key[slot] = last;
					if (last == 0) {
						this.nulll = slot;
	...
	protected void fixPointers(final int i) {
		switch (this.size) {
			...
			default -> {
				...
				if (i == this.first) {
					while (key[++this.first] == 0 && this.first != nulll) {
					}
				} else if (i == this.last) {
					while (key[--this.last] == 0 && this.last != nulll) {
					}
				}
			}
		}
	...
	public void forEach(final IntConsumer action) {
		...
		while (next != -1) {
			curr = next;
			while (++next < last && (key[next] == 0 && next != nulll)) {
			}
			if (last < next) {
				next = -1;
			}
	...
	@Override
	public IntSortedSparseArraySet clone() {
		final IntSortedSparseArraySet c;
		try {
			c = (IntSortedSparseArraySet) super.clone();
	...
	protected long sparseBinarySearch(final int k) {
		final int[] key = this.key;
	...
	private final class SetIterator implements IntBidirectionalIterator {
		int prev = -1;
		int next = -1;
		int curr = -1;

		SetIterator() {
			this.next = IntSortedSparseArraySet.this.first;
		}

		SetIterator(final int from) {
			final int[] key = IntSortedSparseArraySet.this.key;
			switch (IntSortedSparseArraySet.this.size) {
				...
				case 1 -> {
					...
					final int compare = IntSortedSparseArraySet.this.compare(from, key[first]);
				...
				case 2 -> {
					...
					compare = IntSortedSparseArraySet.this.compare(from, key[first]);
					if (compare < 0) { // k < first
						throw new NoSuchElementException("The key " + from + " does not belong to this set.");
					} else if (compare == 0) { // k == first
						this.prev = first;
					...
					compare = IntSortedSparseArraySet.this.compare(key[last], from);
					if (compare < 0) { // last < k
						throw new NoSuchElementException("The key " + from + " does not belong to this set.");
					} else if (compare == 0) { // k == last
						this.prev = last;
				...
				default -> {
					final int nulll = IntSortedSparseArraySet.this.nulll;
					final int first = IntSortedSparseArraySet.this.first;
					final int last = IntSortedSparseArraySet.this.last;
					...
					if (compare < 0) { // k < first
						...
						while (key[++this.next] == 0 && this.next != nulll) {
						}
						return;
					}
					...
					final long packed = IntSortedSparseArraySet.this.sparseBinarySearch(from);
					final int pos = (int) packed;
					if (pos == (int) (packed >>> Integer.SIZE)) {
						this.prev = this.next = pos;
						while (++this.next < last && (key[this.next] == 0 && this.next != nulll)) {
						}
						return;
					}
		...
		public void remove() {
			...
			if (this.next == -1) {
				IntSortedSparseArraySet.this.last = this.prev;
			}
