...
// 	template<typename T, typename A=std::allocator<T>, typename M=std::uintptr_t>
// 	class DynamicArray {
// 		using allocator_type = A;
// 		using value_type = T;
//
// 		using MaskAllocator = std::allocator_traits<A>::template rebind_alloc<M>;
// 		using Mask = mask::ArrayMask<M, MaskAllocator>;
//
// 		using Pointer = std::allocator_traits<A>::pointer;
//
// 		struct Deleter;
// 		using Array = std::unique_ptr<Pointer, Deleter>;
//
//
// 		constexpr static Array makeArray(DynamicArray<T, A, M>& array) {
// 			return Array{nullptr, Deleter{array, ZERO}};
// 		}
//
// 		constexpr static Array makeArray(DynamicArray<T, A, M>& array, const std::size_t n) {
// 			return Array{std::allocator_traits<A>::allocate(array.allocator_, n), Deleter{array, n}};
// 		}
//
// 		constexpr static Array copyArray(DynamicArray<T, A, M>& array, const Array& other) {
...
//
// 		template<bool Move>
// 		constexpr static Array moveArray(DynamicArray<T, A, M>& array, Array&& other) {
// 			if (other.get() == nullptr) {
// 				return makeArray(array);
// 			}
// 			if constexpr (Move) {
...
// 			Mask mask;
//
// 			constexpr Deleter() : allocator{}, size{ZERO}, mask{} {}
//
// 			constexpr Deleter(const A& allocator) : allocator{allocator}, size{ZERO}, mask{} {}
//
// 			constexpr void operator()(T* const pointer) noexcept {
// 				for (std::size_t i{ZERO}; i != this->size; ++i) {
// 					if (this->mask.isSet(i)) {
// 						std::allocator_traits<A>::destroy(this->allocator, std::addressof(pointer[i]));
// 					}
// 				}
// 				std::allocator_traits<A>::deallocate(this->allocator, pointer, this->size);
// 			}
// 		};
//
// 		public:
// 			constexpr DynamicArray() : array_{nullptr, Deleter{}} {}
//
