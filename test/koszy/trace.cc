...
namespace koszy::trace {
	...
	constexpr static std::size_t _TRACES{8U};
	...
	thread_local std::inplace_vector<std::pair<const void*, std::pmr::stacktrace>, _TRACES> _stacktraces{};

	inline auto predicate(const void* pointer) {
		return [pointer](const std::pair<const void*, std::pmr::stacktrace>& pair) -> bool { return pair.first == pointer; };
	}
	...
	extern "C" void __wrap___cxa_throw(void* thrown_object, std::type_info* tinfo, void (*dest)(void*)) {
		if (std::find_if(_stacktraces.cbegin(), _stacktraces.cend(), predicate(thrown_object)) == _stacktraces.cend()) {
			_stacktraces.try_emplace_back(thrown_object, std::pmr::stacktrace::current(1U, _allocator));
		}
		__real___cxa_throw(thrown_object, tinfo, dest);
	}

	std::optional<std::pmr::stacktrace> _trace(const void* object) noexcept {
		std::inplace_vector<std::pair<const void*, std::pmr::stacktrace>, _TRACES>::iterator it{std::remove_if(_stacktraces.begin(), _stacktraces.end(), predicate(object))};
